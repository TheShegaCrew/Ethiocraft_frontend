    ...
    try {
      ...
      const res = await fetch(`${BASE_URL}/auth/register`, {
        ...
        body: JSON.stringify({
          ...
          role: 'ARTISAN',
          artisanProfile: { shopName: values.shopName },
        }),
      });
      ...
      if (!res.ok) {
        const msg = json?.message ?? 'Failed to create artisan account.';
        setErrorMessage(msg);
        return;
      }

      router.push(`/auth/verify-otp?email=${encodeURIComponent(values.email)}&role=ARTISAN`);
    } catch {
            ...
            <section className="w-full md:w-1/2 p-8 md:p-14 flex flex-col justify-between relative z-10">
              <div>
                ...
                <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
                  {/* Full Name */}
                  <div>
                    <label className="font-aeonik block text-xs text-gray-600 mb-1.5 ml-4">
                      Full Name
                    </label>
                    ...
                    {errors.fullName && (
                      <p className="font-aeonik mt-1 ml-4 text-xs text-red-600">
                  ...
                  {errorMessage && (
                    <p
                      role="alert"
                      className="font-aeonik bg-red-50 text-red-600 px-4 py-2 rounded-xl text-sm border border-red-100 mt-4"
                    >
                      {errorMessage}
                    </p>
                  )}
                  ...
                  >
                    {isSubmitting ? 'Creating artisan account...' : 'Create Artisan Account'}
                  </button>
                </form>

                <div className="mt-8 border-t border-gray-300 pt-6">
                  <p className="font-aeonik text-sm text-gray-700 mb-2">
                    Looking to shop handmade products?
                  </p>
